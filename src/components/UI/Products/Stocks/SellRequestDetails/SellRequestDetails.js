...
const SellRequestDetails = ({ isMobileView }) => {
	...
	const { data, isLoading, error, refetch } = useGetStockSellRequestQuery(searchInfo);
	...
	const handleSendSelectedStocks = async () => {
		...
		const fetchData = async () => {
			const response = await fetch(
				`${process.env.NEXT_PUBLIC_BASE_API_URL}/product/selected-stock-session/${process.env.NEXT_PUBLIC_SELL_REQ}`,
				{
					method: 'POST',
					credentials: 'include',
					headers: {
		...
		router.push({
			pathname: '/stocks/sell-stocks/stock-units',
		});
	};
							...
							data?.data?.data?.map((item, index, array) => (
								<tr key={item?.FIN_PROD_ID} className={index === array.length - 1 ? 'custom-border-bottom' : ''}>
									...
									<td className="table-tr-padding profile-value-td text-center">
										<p
											className={`mb-0 text-dark font-w-500 ${styles.ProductTableDatavalue}`}
										>
									...
									<td className="table-tr-padding profile-value-td text-center">
										<div className="d-flex justify-content-center align-items-center">
											<p
												className={`mb-0 ${styles.ProductTableDatavalue}
												${
													item?.GAIN_LOSS >= 0
