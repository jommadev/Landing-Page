	...
	const handleSendSelectedStocks = async () => {
		...
		const fetchData = async () => {
			const response = await fetch(
				`${process.env.NEXT_PUBLIC_BASE_API_URL}/product/selected-stock-session/${process.env.NEXT_PUBLIC_BUY_REQ}`,
				{
					method: 'POST',
					credentials: 'include',
					headers: {
		...
		router.push({
			pathname: '/stocks/buy-stocks/stock-units',
		});
	};
		...
		<>
			...
			<div className="mt-4">
				<p
					className={
						activeIndex === 0
							? `mb-0 ${styles.filterBtnActive}`
							: `mb-0 ${styles.filterBtnInactive}`
					}
				...
				<p
					className={
						activeIndex === 1
							? `mb-0 mx-lg-3 mx-2 ${styles.filterBtnActive}`
							: `mb-0 mx-lg-3 mx-2 ${styles.filterBtnInactive}`
					}
				...
				<table className="table table-striped custom-stripe" id="my-custom-table">
					<thead style={{ backgroundColor: '#2C7C7A' }}>
						<tr>
							...
							<th className="table-header-history">Price (Tk)</th>
							<th
							...
							data?.data?.data?.map((item, index, array) => (
								<tr key={item?.FIN_PROD_ID} className={index === array.length - 1 ? 'custom-border-bottom' : ''}>
									...
									<td className="table-tr-padding profile-value-td text-center">
										<p className={`mb-0 ${styles.stockBuyProductName} text-dark font-w-500 ${styles.ProductTableDatavalue}`}>
									...
									>
										{selectedItems.some(
											...
											<Image
												src={addedStock}
												alt="remove stock"
												className='add-product-image-size add-product-image-size'
					...
					<div className={`${styles.fixedButton}`}>
						<div className="d-flex justify-content-between align-items-center">
							<div className="d-flex align-items-center">
								<p className={`${styles.countSelectedStocks}`}>
