	...
	const handleSendSelectedStocks = async () => {
		const fetchData = async () => {
			const response = await fetch(
				`${process.env.NEXT_PUBLIC_BASE_API_URL}/product/selected-bond-session/${process.env.NEXT_PUBLIC_SELL_REQ}`,
				{
					method: 'POST',
					credentials: 'include',
					headers: {
		...
		router.push({
			pathname: '/bonds/sell-bonds/bond-units',
		});
	};
					...
					<div className={`${styles.fixedButton}`}>
						<div className="d-flex justify-content-between align-items-center">
							<div className="d-flex align-items-center">
								...
								<p className="ms-2 mb-0">Bonds Selected</p>
							</div>
							<ButtonPrimary
