	...
	const handleSendSelectedStocks = async () => {
		const fetchData = async () => {
			const response = await fetch(
				`${process.env.NEXT_PUBLIC_BASE_API_URL}/product/selected-mutualfunds-session/${process.env.NEXT_PUBLIC_SELL_REQ}`,
				{
					method: 'POST',
					credentials: 'include',
					headers: {
		...
		router.push({
			pathname: '/mutual-funds/sell-mutual-funds/mutual-fund-units',
		});
	};
							...
							data?.data?.data?.map((item, index, array) => (
								<tr key={item?.FIN_PROD_ID} className={index === array.length - 1 ? 'custom-border-bottom' : ''}>
									<td className="table-tr-padding profile-value-td">
										...
										Unit(s): {formatNumberWithCommasAndInt(item?.PROD_UNITS)}
										</p>
									</td>
