...
import { useGetStockSellRequestQuery } from '@/redux/api/apiSlice';
...
const SellRequestDetails = ({ isMobileView }) => {
	...
	const { data, isLoading, error, refetch } =
		useGetStockSellRequestQuery(searchInfo);
	...
	const handleSendSelectedStocks = async () => {
		...
		const fetchData = async () => {
			const response = await fetch(
				`${process.env.NEXT_PUBLIC_BASE_API_URL}/product/selected-investment-themes-session/${process.env.NEXT_PUBLIC_SELL_REQ}`,
				{
					method: 'POST',
					credentials: 'include',
					headers: {
		...
		router.push({
			pathname: '/investment-themes/sell-investment-themes/investment-themes-units',
		});
		setIsDisabled(false);
	};
			...
			<div className="table-responsive container mt-4 px-0">
				<table className="table table-striped custom-stripe" id="my-custom-table">
					<thead style={{ backgroundColor: '#2C7C7A' }}>
						<tr>
							<th className="table-header-history text-start" >
								Product Name
							</th>
							<th
							...
							data?.data?.data?.map((item, index, array) => (
								<tr key={item?.FIN_PROD_ID} className={index === array.length - 1 ? 'custom-border-bottom' : ''}>
									<td className="table-tr-padding profile-value-td">
										<p className={`mb-1 ${styles.stockBuyTreadCode}`} style={{textDecoration:'none'}} >
											{item?.TRADE_CODE}
											{item?.IS_FLOOR_FLAG ? (
                                        ...
                                        Value: Tk {formatNumberWithCommasAndDecimal(parseFloat(item?.PROD_UNITS))}
										</p>
									</td>
				...
				<p style={{ fontSize: '12px', color: '#D60D0D' }}>
				Disclaimer: Capital market investment is subject to market risk. Past
					...
					<div className={`${styles.fixedButton}`}>
						<div className="d-flex justify-content-between align-items-center">
							<div className="d-flex align-items-center">
								...
								<p className="ms-2 mb-0">Theme Selected&nbsp;</p>
							</div>
							<ButtonPrimary
