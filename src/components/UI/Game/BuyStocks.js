...
import addedStock from '@/assets/images/game-add-complete-stock-icon.svg';
...
import { useGetGameStocksListQuery } from '@/redux/api/apiSlice';
...
const BuyStocks = ({ isMobileView }) => {
	...
	useGetGameStocksListQuery(searchInfo);
	...
	const handleSendSelectedStocks = async () => {
		const fetchData = async () => {
			const response = await fetch(
				`${process.env.NEXT_PUBLIC_BASE_API_URL}/game/selected-game-stock-session/${process.env.NEXT_PUBLIC_BUY_REQ}`,
				{
					method: 'POST',
					credentials: 'include',
					headers: {
		...
		router.push({
			pathname: '/trading-game/stocks/buy-stocks/stock-units',
		}); 
	};
				...
				>
					<thead>
						<tr>
							...
							<th className="table-header-history" onClick={handleSorting} style={{ cursor: 'pointer' }}>
								Year to Date (%)
								<span>
							...
							data?.data?.data?.map((item, index, array) => (
								<tr key={item?.FIN_PROD_ID} className={index === array.length - 1 ? 'custom-game-border-bottom' : ''}>
									<td className="game-value-td">
										<p className="mb-1 game-stock-trade-code">
											...
											<Image src={item?.FIN_ID === 5 ? BdFlag:  UsaFlag } alt='flag' className='game-filter-flag-product'/>
										</p>
									...
									<td className="game-value-td game-table-value text-center">
									...
									>
										{selectedItems.some(
											...
											<Image src={addedStock} alt="remove stock" className="game-stock-add-icon add-product-image-size" />
										) : (
											<Image src={addStock} alt="add stock" className="game-stock-add-icon" />
										)}
									</td>
								</tr>
							))
